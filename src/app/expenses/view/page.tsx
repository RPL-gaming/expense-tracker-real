            ...
            {!!expenses &&
              expenses.map((expense, index) => (
                <tr key={expense.id} className="hover">
                  ...
                  <td className="text-lg">{new Date(expense.date).toLocaleDateString("en-US", { day: "2-digit", month: "2-digit", year: "numeric" })}</td>
                  <td className="text-lg">{expense.description}</td>
                  <td>
                    <div className="">
                      <button
                        className='text-white bg-purple-700 w-24 h-12 rounded-xl'
                        onClick={() => router.push(`/expenses/edit?id=${expense.id}`)}
                      >
                        Update
                      </button>
                    </div>
